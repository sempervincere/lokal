...
function BOConsultationChat({ cluster: c, sessionId, onBack, expiresAt, initialMsgs, onMsgsChange, isExpired: sessionIsExpired }: { cluster: ClusterData; sessionId: string | null; onBack: () => void; expiresAt: string | null; initialMsgs: any; onMsgsChange: any; isExpired?: boolean }) {
  const [msgs, setMsgs] = useState<Array<{ id: number; role: 'ai' | 'user'; text: string }>>(initialMsgs || [{
    id: 0, role: 'ai',
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '14px 24px', background: `linear-gradient(135deg, ${T.g900} 0%, #1e1b4b 100%)`, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: T.c50 }}>{sessionIsExpired ? 'Riwayat Konsultasi' : 'Konsultasi AI'} — {c.name}</div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', display: 'flex', alignItems: 'center', gap: 5 }}>
              <div style={{ width: 5, height: 5, borderRadius: '50%', background: (expired || sessionIsExpired) ? '#EF4444' : '#4ADE80' }} />
        ...
        {(expired || sessionIsExpired) ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#f8fafc', padding: '4px 4px 4px 12px', borderRadius: 9999, border: `1px solid ${T.c200}` }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontSize: 11, fontWeight: 700, color: T.g900 }}>Custom Mode</span>
                  <button 
                    onClick={() => setCustomMode(!customMode)}
                    style={{
                      width: 28, height: 16, borderRadius: 9999, background: customMode ? T.p600 : T.c200, 
                      border: 'none', cursor: 'pointer', position: 'relative', transition: 'background 200ms'
                    }}
                  >
                    <div style={{
                      width: 12, height: 12, borderRadius: '50%', background: '#fff', 
                      position: 'absolute', top: 2, left: customMode ? 14 : 2, transition: 'left 200ms'
                    }} />
                  </button>
                </div>
                <div style={{ width: 1, height: 14, background: T.c200 }} />
                <div style={{ display: 'flex', gap: 2, opacity: customMode ? 1 : 0.4, pointerEvents: customMode ? 'auto' : 'none', transition: 'opacity 200ms' }}>
                  {[
                    { id: 'motivated', label: 'Motivated' },
                    { id: 'realistic', label: 'Realistic' },
                    { id: 'strategic', label: 'Strategic' }
                  ].map(p => (
                    <button key={p.id} onClick={() => setPersona(p.id as any)} style={{
                      padding: '4px 10px', borderRadius: 9999, border: 'none', cursor: 'pointer', fontFamily: 'inherit',
                      fontSize: 11, fontWeight: persona === p.id && customMode ? 700 : 500,
